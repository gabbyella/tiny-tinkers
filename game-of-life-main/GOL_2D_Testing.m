...
ht=10;                 % set dimensions of cell matrix         
wd=10;
t=.5;                    % set time between each generation (in seconds)
...
for k=1:ht  
    for l=1:wd
        if A(k,l)==1        % in a living cell, if it has...
            if C(k,l)<3     % less than __ living neighbors, it dies
                A(k,l)=0;
            elseif C(k,l)>3 % more than __ living neighbors, it dies
                A(k,l)=0;
            end             % otherwise, it continues to live
        elseif A(k,l)==0    % in a dead cell, if it has...
            if C(k,l)==3    % exactly __ living neighbors, becomes live
                A(k,l)=1;
